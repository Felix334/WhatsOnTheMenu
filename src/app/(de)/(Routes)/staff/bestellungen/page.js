...
// Aus dem Woerterbuch gebaut statt fest verdrahtet — die Schluessel
// (pending/confirmed/done) sind DB-Werte und bleiben unveraendert.
const statusLabels = (to) => ({ pending: to.new, confirmed: to.inProgress, done: to.done });
const STATUS_COLOR = {
  pending: "bg-amber-100 text-amber-800 border-amber-200",
  confirmed: "bg-blue-100 text-blue-800 border-blue-200",
};

function OrderCard({ order, onStatusChange, to }) {
    ...
    <div className={`bg-white rounded-2xl border shadow-sm p-5 space-y-3 ${order.status === "pending" ? "border-amber-300" : "border-gray-200"}`}>
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <span className={`ml-3 text-xs font-medium px-2 py-0.5 rounded-full border ${STATUS_COLOR[order.status]}`}>{statusLabels(to)[order.status]}</span>
        </div>
      ...
      <div className="max-w-2xl mx-auto space-y-6">
        ...
        {orders.length === 0 ? (
          <div className="text-center py-20 text-gray-400">
            ...
            <p className="text-sm mt-1">{to.autoRefresh}</p>
          </div>
        ) : (
          <>
            {pending.length > 0 && (
              <div className="space-y-3">
                ...
                {pending.map((o) => (
                  <OrderCard key={o.id} order={o} onStatusChange={handleStatusChange} to={to} />
                ))}
              </div>
            )}
            {confirmed.length > 0 && (
              <div className="space-y-3">
                <h2 className="text-sm font-semibold text-blue-700 uppercase tracking-wide">{to.inProgress} ({confirmed.length})</h2>
                {confirmed.map((o) => (
                  <OrderCard key={o.id} order={o} onStatusChange={handleStatusChange} to={to} />
                ))}
              </div>
...
export default function Bestellungen() {
  ...
  if (status === "loading") {
    return <div className="min-h-screen flex items-center justify-center">{tc.loading}</div>;
  }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50 p-4">
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-8 max-w-md w-full text-center space-y-4">
          ...
          <h2 className="text-xl font-bold text-gray-900">{tc.proRequired}</h2>
