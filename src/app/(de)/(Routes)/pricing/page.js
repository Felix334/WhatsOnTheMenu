...
import { SUPPORTED_COUNTRIES } from "@/lib/schemas/restaurant";
import { isTierAvailableUI } from "@/lib/tierAvailability";

// Anzeige-Schalter pro Tarif. Die Liste steht jetzt zentral in
// src/lib/tierAvailability.js, damit Registrierungsformular und Tarifseite
// nicht auseinanderlaufen. Verbindlich bleibt AVAILABLE_TIERS in
// src/lib/stripe.ts, serverseitig erzwungen in api/payment/checkout.
const TIER_AVAILABLE_UI = {
  Professional: isTierAvailableUI("Professional"),
  Business: isTierAvailableUI("Business"),
};

// Ländernamen nicht hart hinterlegen, sondern vom Browser benennen lassen:
// Intl.DisplayNames liefert sie in der jeweiligen Sprache. Wenn die
// Registrierungsstrecke später übersetzt wird, stimmt die Liste automatisch
// mit, ohne dass jemand 31 Namen pro Sprache pflegt.
function useCountryOptions(locale = "de") {
  return useMemo(() => {
    let naming;
    try {
...
export default function PricingPage() {
  ...
  const countryOptions = useCountryOptions("de");
                ...
                <Dialog open={showProForm} onOpenChange={setShowProForm}>
                  ...
                  <DialogContent className="max-w-2xl h-[80vh]">
                    ...
                    <form onSubmit={handleProSubmit} className="space-y-4 overflow-y-auto">
                      ...
                      <div className="space-y-2">
                        <Label>Land *</Label>
                        <Select value={restaurantData.country} onValueChange={(value) => updateRestaurantData("country", value)}>
                          <SelectTrigger>
                            <SelectValue />
                          </SelectTrigger>
                          <SelectContent>
                        ...
                        <p className="text-xs text-gray-500">Bestimmt die Umsatzsteuer auf Ihrer Rechnung.</p>
                      </div>
